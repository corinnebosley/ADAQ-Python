#!/usr/bin/env python
"""
Generalised script to generate a plot of the number of ensemble members
...
.. code-block:: ksh
   ...
   ./ens_exceedance_plot.py [inifilename]
...
.. code-block:: python
    ...
    from adaqscripts import ens_exceedance_plot
    ens_exceedance_plot.exceedance_plotting(inifilename='inifilename')
...
.. code-block:: python
    ...
    ini_dict = {}
    ini_dict['key'] = value
    ens_exceedance_plot.exceedance_plotting(ini_dict=ini_dict)
...
|               |respectively.                                                 |
+---------------+--------------------------------------------------------------+
|threshold_list |List of thresholds to calculate the number of ensemble        |
|               |members which exceed the threshold. The threshold should be   |
|               |in the units specified in "units".                            |
+---------------+--------------------------------------------------------------+
|units          |units (e.g. units = 'ug/m3') of the thresholds specified in   |
|               |"threshold_list".                                             |
+---------------+--------------------------------------------------------------+
...
|                    |columns to be plotted. Note that only 3 levels can be    |
|                    |listed due to memory contraints.                         |
+--------------------+---------------------------------------------------------+
...
def check_ini_dict(ini_dict):
    ...
    if not 'units' in ini_dict:
        raise ValueError('"units" must be set in the inifile.')

    if not 'threshold_list' in ini_dict:
        raise ValueError('"threshold_list" must be set in the inifile.')
