...
class WindroseAxes(PolarAxes):
    ...
    def set_radii_angle(self, normed, **kwargs):
        """
        ...
        rmax = self.get_rmax()
        # Get the number of digits in the number minus one returned by get_rmax
        l = int(math.log10(rmax))
        # Create a number 10, 100, 1000 etc depending on the number of 
        # digits returned by get_rmax
        num = int('1' + '0' * l)
        # Calculate the maximum value for the plot. If rmax is 37 then 
        # the maximum is 40. If rmax is 370 then the maximum is 400.
        top = int(math.ceil(old_div(rmax,num))*num)
        # The number of labels depends on the first digit of the maximum.
        numLabels = (old_div(top, num))
        # If this is less than 4 then double the number of grid lines
        if numLabels < 4:
            numLabels *= 2

        radii = np.linspace(0.001, top, numLabels+1)
        if normed:
            radii_labels = [ "%d%s" %(r, '%') for r in radii ]
        else:
    ...
    def _update(self, normed):
        self.set_rmax(rmax=np.max(np.sum(self._info['table'], axis=0)))
        self.set_radii_angle(normed, angle=self.radii_angle)


    def legend(self, units, loc='lower left', **kwargs):
        """
    ...
    def _init_plot(self, dir, var, **kwargs):
        ...
        angles = np.arange(0, -2*np.pi, -2*np.pi/nsector) + old_div(np.pi,2)
    ...
    def bar(self, dir, var, **kwargs):
        ...
        for j in range(nsector):
            ...
            for i in range(nbins):
                ...
                patch = Rectangle((angles[j]-old_div(opening,2), offset), opening, val,
        ...
        patch = Polygon(vertices, closed = True,
                facecolor=colors[0], edgecolor=edgecolor, zorder=ZBASE + nbins + 1,
                **kwargs)
        self.add_patch(patch)
        ...
        if percflag:
            self.annotate("%.1f%s" % ((old_div(table2[0,:].sum(),table2.sum()))*100, '%'), xy=(0,0), xytext=(0,0), ha="center", va="center")

        else:
            self.annotate("%.1f" % table2[0,:].sum(), xy=(0,0), xytext=(0,0), ha="center", va="center")
...
def histogram(dir, var, bins, nsector, normed=False, blowto=False):
    ...
    dir_bins = np.arange(old_div(-angle,2) ,360.+angle, angle, dtype=np.float)
